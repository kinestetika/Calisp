...
def load_unicode():
        for line in unimod.UNIMOD.splitlines():
            ...
            if m:
                ...
                __ALL_UNIMOD_PEPTIDE_MODIFICATIONS_IDS.append(id)
                extra_neutrons = 0
                if len(words) > 2:
                    extra_neutrons = np.int16(int(words[2]))
                __ALL_UNIMOD_PEPTIDE_MODIFICATIONS_EXTRA_NEUTRONS.append(extra_neutrons)
        #print(f'parsed {len(__ALL_UNIMOD_PEPTIDE_MODIFICATIONS_ELEMENT_COUNTS)} unimod entries')
        #for k in __ALL_UNIMOD_PEPTIDE_MODIFICATIONS_ELEMENT_COUNTS:
         #   print(f'"{k}" => {__ALL_UNIMOD_PEPTIDE_MODIFICATIONS_ELEMENT_COUNTS[k]}')
...
def unimod_peptide_modifications_id(id):
    if not __ALL_UNIMOD_PEPTIDE_MODIFICATIONS_IDS:
        load_unicode()
    for i in range(len(__ALL_UNIMOD_PEPTIDE_MODIFICATIONS_IDS)):
        if __ALL_UNIMOD_PEPTIDE_MODIFICATIONS_IDS[i] == id:
            return i
    return -1


def compute_peptide_mass(aminoacid_sequence: str, peptide_modifications, element_counts = np.zeros(5, dtype=np.int16)): # assumed to be uppercase
    for i in range(5):
...
def molecular_formula(element_counts):
    return f"C{element_counts[0]}" \
           f"H{element_counts[3]}" \
           f"N{element_counts[1]}" \
           f"O{element_counts[2]}" \
           f"S{element_counts[4]}"
